package apt

import (
	"bufio"
	"compress/gzip"
	"context"
	"fmt"
	"io"
	"net/http"
	"net/url"
	"os"
	"slices"
	"strings"

	"github.com/aws/aws-sdk-go-v2/config"
	"github.com/aws/aws-sdk-go-v2/service/s3"
	"pault.ag/go/debian/control"
)

func readIndex(url string) ([]Package, map[string]string, error) {
	if strings.HasPrefix(url, "s3://") {
		return readS3Index(url)
	} else if strings.HasPrefix(url, "http://") || strings.HasPrefix(url, "https://") {
		return readHTTPIndex(url)
	}

	return readFile(url)
}

func readS3Index(s3URL string) ([]Package, map[string]string, error) {
	u, err := url.Parse(s3URL)
	if err != nil {
		return nil, nil, err
	}

	u.Path = strings.TrimPrefix(u.Path, "/")

	cfg, err := config.LoadDefaultConfig(context.Background())
	if err != nil {
		return nil, nil, fmt.Errorf("failed to load S3 configuration, %w", err)
	}

	client := s3.NewFromConfig(cfg)

	obj, err := client.GetObject(context.Background(), &s3.GetObjectInput{
		Bucket: &u.Host,
		Key:    &u.Path,
	})
	if err != nil {
		return nil, nil, err
	}

	return parseIndex(obj.Body, strings.HasSuffix(u.Path, ".gz"))
}

func readHTTPIndex(url string) ([]Package, map[string]string, error) {
	resp, err := http.Get(url) //nolint:gosec,noctx
	if err != nil {
		return nil, nil, err
	}

	return parseIndex(resp.Body, strings.HasSuffix(url, ".gz"))
}

func readFile(path string) ([]Package, map[string]string, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, nil, err
	}

	return parseIndex(f, strings.HasSuffix(path, ".gz"))
}

func parseIndex(r io.ReadCloser, compressed bool) ([]Package, map[string]string, error) {
	index, err := readPackageIndex(r, compressed)
	if err != nil {
		return nil, nil, err
	}

	var packages []Package

	aliases := make(map[string]string)

	for _, entry := range index {
		if _, ok := entry.Values["XB-Softpack"]; !ok {
			continue
		}

		if alias, ok := entry.Values["XB-Alias"]; ok {
			aliases[alias] = entry.Package
			entry.Package = alias
		}

		pos, exists := slices.BinarySearchFunc(packages, Package{Name: entry.Package}, func(a, b Package) int {
			return strings.Compare(a.Name, b.Name)
		})
		if !exists {
			packages = slices.Insert(packages, pos, Package{Name: entry.Package, Description: entry.Description})
		}

		packages[pos].Versions = append(packages[pos].Versions, entry.Version.Version)
	}

	return packages, aliases, nil
}

func readPackageIndex(r io.ReadCloser, compressed bool) ([]control.BinaryIndex, error) {
	defer r.Close()

	if compressed {
		s, err := gzip.NewReader(r)
		if err != nil {
			return nil, err
		}

		r = s
	}

	return control.ParseBinaryIndex(bufio.NewReader(r))
}
